...
def plotHeatLSS(adata, latent, allCellTypes, ctPairs, save=False, plot=False):
...
def getBatchCellDicts(adata, cellTypeLabel, colors=plt.get_cmap('tab10').colors, shapes="ospx^><....."):
	...
	if len(colors) < max(len(adata.obs[cellTypeLabel].cat.categories), len(adata.obs["overlapLabel"].cat.categories)):
		colors = colors + [colors[0]]*max(len(adata.obs[cellTypeLabel].cat.categories), len(adata.obs["overlapLabel"].cat.categories))
